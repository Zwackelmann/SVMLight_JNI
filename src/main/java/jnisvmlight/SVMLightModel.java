...
public class SVMLightModel implements Serializable {
    private static final long serialVersionUID = -7564359001092444117L;

    /**
     * Reads an SVM-light model from a URL and creates an SVMLightModel object
     * in Java. The format is compatible to the SVM-light model files.
     */
    public static SVMLightModel readSVMLightModelFromURL(URL file) throws ParseException {
        ...
        try {
            ...
            linecnt++;
            line = bi.readLine();
            kType = new Long((line.substring(0, line.indexOf('#'))).trim()).longValue();
            linecnt++;
            line = bi.readLine();
            dParam = new Long((line.substring(0, line.indexOf('#'))).trim()).longValue();
            linecnt++;
            line = bi.readLine();
            gParam = new Double((line.substring(0, line.indexOf('#'))).trim()).doubleValue();
            linecnt++;
            line = bi.readLine();
            sParam = new Double((line.substring(0, line.indexOf('#'))).trim()).doubleValue();
            linecnt++;
            line = bi.readLine();
            rParam = new Double((line.substring(0, line.indexOf('#'))).trim()).doubleValue();
            linecnt++;
            line = bi.readLine();
            uParam = (line.substring(0, line.indexOf('#'))).trim();
            linecnt++;
            line = bi.readLine();
            highFeatIdx = new Long((line.substring(0, line.indexOf('#'))).trim()).longValue();
            linecnt++;
            line = bi.readLine();
            trainDocs = new Long((line.substring(0, line.indexOf('#'))).trim()).longValue();
            linecnt++;
            line = bi.readLine();
            numSupVecs = new Long((line.substring(0, line.indexOf('#'))).trim()).longValue();
            linecnt++;
            line = bi.readLine();
            threshold = new Double((line.substring(0, line.indexOf('#'))).trim()).doubleValue();
        ...
        SVMLightModel model = new SVMLightModel(format, kType, dParam, gParam,
                sParam, rParam, uParam, highFeatIdx, trainDocs, numSupVecs,
                threshold, lfv);
    ...
    protected SVMLightModel(String format, long ktype, long dparm,
            double gparm, double sparm, double rparm, String uparm, long hfi,
            long tdocs, long nsv, double threshold, LabeledFeatureVector[] docs) {
        ...
        case 0:
            ...
            break;
        case 1:
            m_kernel = new PolynomialKernel(new LinearKernel(), m_dParam, m_sParam, m_rParam);
            break;
        case 2:
    ...
    public double classify(FeatureVector v) {
        ...
        if (m_kType == 0) {
            ...
            for (int i = 0; i < m_docs.length; i++) {
                ...
                if (alpha != 0) {
                    delta += v.m_factor * alpha
                            * m_kernel.evaluate(m_docs[i], v);
                }
            }
        }
